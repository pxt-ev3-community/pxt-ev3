...
namespace pxsim.visuals {
    export class NXTTouchSensorView extends ModuleView implements LayoutElement {
        ...
        constructor(port: number) {
            super(TOUCH_SENSOR_SVG, "touch", NodeType.NXTTouchSensor, port);
        }
        ...
        private setPressed(pressed: boolean) {
            if (pressed) {
                for (let i = 0; i < 4; i ++) {
                    this.setStyleFill(`${this.normalizeId(NXTTouchSensorView.RECT_ID[i])}`, `${this.normalizeId(NXTTouchSensorView.TOUCH_GRADIENT_PRESSED[i])}`, NXTTouchSensorView.LIGHT_TOUCH_BLACK_COLOR);
                }
            } else {
                for (let i = 0; i < 4; i ++) {
                    this.setStyleFill(`${this.normalizeId(NXTTouchSensorView.RECT_ID[i])}`, `${this.normalizeId(NXTTouchSensorView.TOUCH_GRADIENT_UNPRESSED[i])}`, NXTTouchSensorView.LIGHT_TOUCH_RED_COLOR);
                }
            }
        }
    }
